...
export default function AdminTeamsPage() {
  ...
  const fetchTeams = async () => {
    setLoading(true);
    try {
      const res = await fetch('http://localhost:5001/api/admin/teams', { credentials: 'include' });
  ...
  const handleDelete = async (teamId) => {
    ...
    try {
      const res = await fetch(`http://localhost:5001/api/admin/teams/${teamId}`, {
        method: 'DELETE',
        credentials: 'include',
      });
  ...
  const handleEditName = async (teamId) => {
    ...
    if (!newName) return;
    try {
      const res = await fetch(`http://localhost:5001/api/admin/teams/${teamId}/name`, {
        method: 'PUT',
  ...
  const handleChangeLeader = async (teamId) => {
    ...
    try {
      const res = await fetch(`http://localhost:5001/api/admin/teams/${teamId}/leader`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
  ...
  const handleAddMember = async (teamId) => {
    ...
    try {
      const res = await fetch(`http://localhost:5001/api/admin/teams/${teamId}/members`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
  ...
  const handleRemoveMember = async (teamId, memberId) => {
    ...
    try {
      const res = await fetch(`http://localhost:5001/api/admin/teams/${teamId}/members/${memberId}`, {
        method: 'DELETE',
        credentials: 'include',
      });
  ...
  const handleJoinRequest = async (teamId, requestId, action) => {
    try {
      const res = await fetch(`http://localhost:5001/api/admin/teams/${teamId}/join/${requestId}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',

...
export default function ProfilePage() {
  ...
  async function saveLinks() {
    setLinksMsg('');
    try {
      const res = await fetch(`/api/users/social/me`, { // This is the correct URL
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
  ...
  return (
    <div className="mx-auto max-w-3xl p-6 text-slate-200 min-h-screen">
      ...
      <div className="mb-6 flex gap-2 border-b border-slate-700">
        <button 
          className={`border-b-2 px-4 py-2 text-sm font-medium ${tab === 'profile' ? 'border-purple-500 text-white' : 'border-transparent text-slate-400 hover:text-slate-200'}`} 
          onClick={() => setTab('profile')}
        >
          Profile
        </button>
        <button 
          className={`border-b-2 px-4 py-2 text-sm font-medium ${tab === 'photo' ? 'border-purple-500 text-white' : 'border-transparent text-slate-400 hover:text-slate-200'}`} 
          onClick={() => setTab('photo')}
        >
          Photo
        </button>
        <button 
          className={`border-b-2 px-4 py-2 text-sm font-medium ${tab === 'links' ? 'border-purple-500 text-white' : 'border-transparent text-slate-400 hover:text-slate-200'}`} 
      ...
      {tab === 'profile' && (
        <section className="space-y-8">
          <form onSubmit={handleProfileSubmit} className="space-y-4">
            ...
            {profileMsg && (
              <div className={`rounded p-3 text-sm ${profileMsg.includes('success') ? 'bg-green-500/20 text-green-300' : 'bg-red-500/20 text-red-300'}`}>
                {profileMsg}
              </div>
            )}
            ...
            <div>
              ...
              <input 
                ...
                className="w-full rounded border border-slate-700 bg-slate-800 px-3 py-2 text-sm disabled:opacity-50" 
              />
              <p className="text-xs text-slate-500 mt-1">
                Changes remaining: {Math.max(0, 2 - (user?.nameUpdateCount || 0))}
              </p>
            </div>
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">
                Changes remaining: {Math.max(0, 4 - (user?.yearUpdateCount || 0))}
              </p>
            </div>

            <button 
              type="submit" 
              className="rounded-lg bg-blue-600 px-5 py-2.5 font-medium text-white transition-transform duration-200 hover:scale-105 active:scale-95"
            >
              Save Profile
            </button>
          </form>
          ...
          <form onSubmit={handlePasswordSubmit} className="space-y-4 border-t border-slate-700 pt-8">
            ...
            {passwordMsg && (
              <div className={`rounded p-3 text-sm ${passwordMsg.includes('success') ? 'bg-green-500/20 text-green-300' : 'bg-red-500/20 text-red-300'}`}>
                {passwordMsg}
              </div>
            )}
      ...
      {tab === 'photo' && (
        <section>
          {photoMsg && (
            <div className="mb-4 rounded p-3 text-sm bg-slate-700 text-white">
              {photoMsg}
            </div>
          )}
          ...
          <div className="mt-4 flex items-center gap-2">
            <input 
              ...
              className="block w-full text-sm text-gray-400 file:mr-4 file:rounded-md file:border-0 file:bg-slate-700 file:px-4 file:py-2 file:text-white hover:file:bg-slate-600" 
            />
          </div>
          <div className="mt-4 flex items-center gap-2">
            <button 
      ...
      {tab === 'links' && (
        <section>
          {linksMsg && (
            <div className="mb-4 rounded p-3 text-sm bg-green-500/20 text-green-300">
              {linksMsg}
            </div>
          )}
          ...
          <div className="space-y-4">
            ...
            <button 
              onClick={saveLinks} 
              className="rounded-lg bg-purple-600 px-5 py-2.5 font-medium text-white transition-transform duration-200 hover:scale-105 active:scale-95"
            >
              Save Links
            </button>
          </div>

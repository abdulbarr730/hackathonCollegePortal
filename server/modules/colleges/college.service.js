...
/* ================= DIRECT ADD STAFF WITH MAX 5 COMBINED LIMIT ================= */
exports.addCollegeStaff = async ({ collegeId, name, email, role = 'spoc', phone = '', password = '', sendEmail = true, clientUrl, requester }) => {
  ...
  // Check if updating existing staff vs adding new
  const existingIdx = (college.staff || []).findIndex(s => s.email === normalizedEmail);

  // ENFORCE MAX 5 SPOC/ADMIN COMBINED LIMIT PER COLLEGE FOR NEW ADDITIONS
  if (existingIdx === -1 && (college.staff || []).length >= 5) {
    throw new ApiError(400, 'Maximum limit reached: A college can have up to 5 SPOCs and Administrators combined.');
  }

  const staffRole = ['spoc', 'college_admin', 'admin', 'judge'].includes(role) ? role : 'spoc';
  const initialPassword = password || `${(college.shortName || 'CampX').replace(/[^a-zA-Z0-9]/g, '')}@${Math.floor(1000 + Math.random() * 9000)}`;
  ...
  if (user) {
    ...
    user = await User.create({
      ...
      isVerified: true,
      college: college._id,
      adminNotes: 'College SPOC / Admin appointed via staff management console.'
    });
  }

  // Update or insert staff entry
  if (existingIdx >= 0) {
    college.staff[existingIdx].name = name;
    college.staff[existingIdx].user = user._id;
    college.staff[existingIdx].role = staffRole;
    college.staff[existingIdx].phone = phone || college.staff[existingIdx].phone || '';
    college.staff[existingIdx].isVerified = true;
    college.staff[existingIdx].verifiedAt = new Date();
  } else {
    college.staff.push({
      user: user._id,
      name,
  ...
  if (sendEmail !== false) {
    ...
    sendMail({
      to: normalizedEmail,
      subject: `Official ${staffRole.toUpperCase()} Access Provisioned - ${college.shortName || college.name}`,
      html: `<!DOCTYPE html>
      <html>
      <head>
        <meta charset="utf-8">
      ...
      <body>
        <div class="container">
          <div class="badge">⚡ Staff Access Provisioned</div>
          <h1>Welcome to ${college.shortName || college.name} Staff Portal</h1>
          <p>Dear <strong>${name}</strong>,</p>
          <p>You have been appointed as an official <strong>${staffRole.toUpperCase()}</strong> for <strong>${college.name}</strong> on CampXCode Portal.</p>
          
          <div class="cred-box">
            <div style="font-weight: 800; font-size: 12px; text-transform: uppercase; color: #4f46e5; margin-bottom: 14px;">Your Staff Credentials</div>
            <div class="cred-row"><span class="cred-label">Portal URL:</span> <span class="cred-val">${baseUrl}</span></div>
            <div class="cred-row"><span class="cred-label">Login Email:</span> <span class="cred-val">${normalizedEmail}</span></div>
            <div class="cred-row"><span class="cred-label">Initial Password:</span> <span class="cred-val">${initialPassword}</span></div>
            <div class="cred-row"><span class="cred-label">Assigned Role:</span> <span class="cred-val">${staffRole.toUpperCase()}</span></div>
          </div>

          <div class="btn-wrap">
            <a href="${loginUrl}" class="btn" target="_blank">Log In to Staff Portal &rarr;</a>
          </div>

          <div class="footer">
            Official institutional notice from CampXCode Portal.<br/>
            &copy; ${new Date().getFullYear()} Campus Hackathon Portal. All rights reserved.
          </div>
        </div>
      </body>
      </html>`
    }).catch(e => console.error('Staff email failed:', e.message));
  }

  return {
    msg: `Successfully appointed ${name} as ${staffRole.toUpperCase()} for ${college.shortName || college.name}`,
    user,
    college
  };
};
